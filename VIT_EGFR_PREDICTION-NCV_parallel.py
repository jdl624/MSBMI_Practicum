...
#$ Load and initialize feature extractor
...
for fold in range(outer_folds):
    ...
    for inner_fold in range(inner_folds):
        ...
        for epoch in range(num_epochs):
            ...
            for data in train_loader:
                ...
                train_proba.extend(p[:,1].detach().cpu().numpy())
        ...
        loss_df.to_csv(f'/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/train&valid_loss_df_NCV_outerloop{fold}_innerfold{inner_fold}.csv', index=False)
        acc_df.to_csv(f'/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/train&valid_acc_df_NCV_outerloop{fold}_innerfold{inner_fold}.csv', index=False)
        acc_df.to_csv(f'/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/train&valid_auc_df_NCV_outerloop{fold}_innerfold{inner_fold}.csv', index=False)
        ...
        plt.savefig(f"/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/ViT_TRAIN_METRICS_NCV_outerloop{fold}_innerfold{inner_fold}.png")
        plt.close()
    ...
    with torch.no_grad():
        for data in test_loader:
            ...
            p = f.softmax(output, dim=1)
            test_proba.extend(p[:,1].detach().cpu().numpy())
            _, predicted_labels = torch.max(output, 1)
    ...
    plt.plot([0,1],[0,1],"k--")
    plt.legend(loc=4)
    plt.savefig(f'/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/ROC_CURVE_outerloop_{fold}.png')
    plt.show()
    ...
    def plot_confusion_matrix(cm, classes,
        ...
        plt.savefig(f"/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/CONFUSION_MATRIX_outerloop{fold}.png")
        plt.close()
    ...
    df.to_csv(f'/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/debugclassification_report_outerloop{fold}.csv', index=True)
...
torch.save(model_state, '/gpfs/scratch/jdl624/EGFR_Mutation_Results/ViT/ViT_Models/best_egfr_ViT-B.pt')
print(f'Complete, congratulations! The best model AUC is {best_auc}. Best model saved as best_egfr_ViT-B.pt')
