...
class WSI_Matcher:
    ...
    @staticmethod
    def get_thumbnails(fixed_wsi_obj, float_wsi_obj, rescale_rate=1):
    ...
    def get_initial_pos(thumbnail_fixed, thumbnail_float, thumbnail_down_rate):
        ...
        try:
            ...
            (E, status) = cv2.estimateAffinePartial2D(np.float32(cv2.KeyPoint_convert(selected_kps_fixed)), np.float32(cv2.KeyPoint_convert(selected_kps_float)),
